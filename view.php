...
?>
	...
	<div class="container" style="padding-top:1em;">
		...
		<div class="text-right" style="padding-bottom: 20px;">
			...
			<?php
				if($text['category'] == '공지사항'){ $list_url = "./notice.php";}
				else if($text['category'] == '자유게시판'){ $list_url = "./board.php";}
			?>
			...
			?>
				...
				<button type="button" class="btn btn-warning" style="border-radius : 0.5em;" onclick="formset();">수정</button>
				<a class="btn btn-danger" style="border-radius : 0.5em;" href="#" onclick="delete_list('<?=$text['no']?>');">삭제</a>
			<?php
				}
			?>
		</div>
		...
		<?php
			...
			$result = $db->query($sql);
			$comment_cnt = $result->fetch_assoc();
		...
		<div class="card" style="margin-bottom: 30px; ">
			<div class="card-body">
			<textarea id="comment" style="height: 57px; width: 1000px; resize: none;" onkeydown="if(event.keyCode==13) comment_insert();"></textarea>
			<button type="button" class="btn btn-primary" style="margin-left: 5px; border-radius: 0.5em; margin-top: -50px; width: 100px; height: 60px;" onclick="comment_insert();">등록하기</button>
			</div>
				...
				<?php if($row_comment['depthid'] == $row_comment['no']) { ?>
					<div class="alert alert-light comment" role="alert">
						...
						<?php if ($row_comment['id'] == $_SESSION['user_name'] || $_SESSION['gm'] == '2'){?> <!--자신이 작성한 글이거나 권한 레벨이 2라면 삭제 권한을 준다 -->
							/ <a onclick="comment_model('<?=$count2?>', '<?=$row_comment['no']?>');">수정</a> / <a onclick="comment_delete('<?=$row_comment['no']?>');">삭제</a> / <a onclick="comment_model2('<?=$row_comment['no']?>');"> 대댓글</a>
						<?php } else{ ?>
				...
				<div style="margin-left: 30px;">
					<div class="alert alert-light comment" role="alert">
						...
						<?php if ($row_comment['id'] == $_SESSION['user_name'] || $_SESSION['gm'] == '2'){?>
							/ <a onclick="comment_model('<?=$count2?>', '<?=$row_comment['no']?>');">수정</a> / <a onclick="comment_delete('<?=$row_comment['no']?>');">삭제</a>
	...
	<!-- 수정시 전송데이터 -->
	<form enctype='multipart/form-data' action='./<?php if($_GET['list'] == 1){ echo "notice_up.php"; } else{ echo "write_up.php";} ?>' method='post' id="board_form">
